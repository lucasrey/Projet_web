...
<nav class="navbar navbar-expand-md fixed-top navbar-light bg-light">
  ...
  <div class="container">
    <p></p>
    <h3 align="center">Suppression : Livre </h3>  
 ...
 {  
   ...
   // $use = mysqli_real_escape_string($connect, $_POST["use"]);  
    ...
    echo $ID; 
      ...
      $query = "SELECT distinct i.ID FROM item i, livre l WHERE i.ID = l.ID_livre AND i.ID_vd = '$ID'"; 
           ...
           {  
                ...
                { 
                  ...
                  $IDLivre = $row['ID'];
                  echo $IDLivre;
                  echo "<option class=form-control value=".$IDLivre.">$IDLivre</option>";
...
if(mysqli_query($connect, $sql)){
	echo '<script>alert("Table Image : Altered")</script>';

}
$sql="DELETE FROM image WHERE ID = '$IDLivre'";
if(mysqli_query($connect, $sql)){
	echo '<script>alert("Image deleted")</script>';

}
$sql="ALTER TABLE livre
ADD CONSTRAINT ID_livre FOREIGN KEY (ID) REFERENCES item(ID) ON DELETE CASCADE";
if(mysqli_query($connect, $sql)){
	echo '<script>alert("Table livre : altered")</script>';
...
$sql="DELETE FROM item WHERE ID = '$IDLivre'";
if(mysqli_query($connect, $sql)){
