...
<nav class="navbar navbar-expand-md fixed-top navbar-light bg-light">
 ...
 <!-- Page Content -->
  <div class="container">
    <p></p>
    <h3 align="center">Suppression : Vetements </h3>  
...
//echo $Pseudo;
 ...
 {  
   ...
   // $use = mysqli_real_escape_string($connect, $_POST["use"]);  
    ...
    $sql = "SELECT p.PersonID from vendeur v, person p where p.PersonID = v.ID AND v.pseudo = '$Pseudo'";
   ...
   // echo $ID; 
      ...
      $query = "SELECT distinct i.ID FROM item i, vetement l WHERE i.ID = l.ID_vetement AND i.ID_vd = '$ID'"; 
...
if(mysqli_query($connect, $sql)){
	echo '<script>alert("Table clothe : Altered")</script>';
...
$sql="ALTER TABLE variation
ADD CONSTRAINT ID_variation FOREIGN KEY (ID) REFERENCES vetement(ID) ON DELETE CASCADE";
if(mysqli_query($connect, $sql)){
  echo '<script>alert("Table variation : altered")</script>';

}

$sql="DELETE FROM variation WHERE ID = '$IDMusique'";
if(mysqli_query($connect, $sql)){
  echo '<script>alert("variation deleted")</script>';

}

$sql="ALTER TABLE vetement
ADD CONSTRAINT ID_vetement FOREIGN KEY (ID) REFERENCES item(ID) ON DELETE CASCADE";
if(mysqli_query($connect, $sql)){
	echo '<script>alert("Table musique : altered")</script>';

}
$sql = "DELETE FROM vetement WHERE ID = '$IDMusique'";
if(mysqli_query($connect, $sql)){
